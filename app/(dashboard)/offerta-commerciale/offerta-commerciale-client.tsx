...
export function OffertaCommercialeClient() {
  ...
  const createLinkedOffer = async () => {
    ...
    if (!newOfferUrl.trim()) {
      toast.error("Inserisci l'URL pubblico")
      return
    }
    setCreatingOffer(true)
    try {
      const response = await fetch("/api/offerta-commerciale/offerte", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          titolo: newOfferTitle,
          tipo: "locandina",
          url_pubblico: newOfferUrl,
          pubblicata: true,
          ordinamento: data?.offerte.length ?? 0,
        }),
      })
      const body = await response.json()
      if (!response.ok) throw new Error(body.error ?? "Creazione offerta non riuscita")
      setNewOfferTitle("")
      setNewOfferUrl("")
      toast.success("Link offerta aggiunto")
      await load()
    } catch (error) { toast.error(error instanceof Error ? error.message : "Errore creazione offerta") }
  ...
  return <div className="flex flex-col gap-6">
    ...
    <Tabs defaultValue="calcolo">
      ...
      <TabsContent value="offerte" className="space-y-4 pt-5">{canManage ? <section className="rounded-xl border border-border bg-card p-4"><div className="mb-3"><h2 className="font-semibold">Aggiungi link pubblico</h2><p className="text-sm text-muted-foreground">Usa questo per locandine o pagine Sanity gia pubbliche sul sito.</p></div><div className="grid gap-3 lg:grid-cols-[minmax(180px,0.7fr)_minmax(260px,1.3fr)_auto]"><Input value={newOfferTitle} onChange={(e) => setNewOfferTitle(e.target.value)} placeholder="Linea MYKONOS" aria-label="Titolo link offerta" /><Input value={newOfferUrl} onChange={(e) => setNewOfferUrl(e.target.value)} placeholder="https://solairgroup.it/linea/MYKONOS" aria-label="URL pubblico offerta" /><Button onClick={createLinkedOffer} disabled={creatingOffer}>{creatingOffer ? <Loader2 className="size-4 animate-spin" /> : null}Aggiungi</Button></div></section> : null}{data.offerte.length === 0 ? <Empty>Carica PDF e copertine in “Offerte-del-periodo” su Nextcloud, oppure aggiungi un link pubblico.</Empty> : <div className="grid gap-4 lg:grid-cols-2">{data.offerte.map((offer) => <article key={offer.id} className="overflow-hidden rounded-xl border border-border bg-card">{offer.cover_path ? <img src={`/api/offerta-commerciale/offerte/${offer.id}/asset?kind=cover`} alt="" className="h-52 w-full object-cover" /> : <div className="flex h-36 items-center justify-center bg-muted"><FileText className="size-10 text-muted-foreground" /></div>}<div className="space-y-3 p-4"><div className="grid gap-2 sm:grid-cols-[1fr_150px]"><Input value={offer.titolo} disabled={!canManage} onChange={(e) => setOffer(offer.id,{titolo:e.target.value})} /><select className="h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground" value={offer.tipo ?? "offerta"} disabled={!canManage} onChange={(e) => setOffer(offer.id,{tipo:e.target.value as OffertaPeriodo["tipo"]})}><option value="offerta">Offerta</option><option value="locandina">Locandina</option><option value="brochure">Brochure</option><option value="finanziaria">Finanziaria</option><option value="pagina">Pagina</option></select></div><Textarea value={offer.descrizione ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{descrizione:e.target.value})} placeholder="Descrizione breve" /><Input value={offer.url_pubblico ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{url_pubblico:e.target.value || null})} placeholder="URL pubblico Sanity/sito" /><div className="grid grid-cols-2 gap-2"><Input type="date" value={offer.valido_dal ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{valido_dal:e.target.value || null})} /><Input type="date" value={offer.valido_al ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{valido_al:e.target.value || null})} /></div><div className="flex items-center justify-between gap-2"><label className="flex items-center gap-2 text-sm"><Switch checked={offer.pubblicata} disabled={!canManage} onCheckedChange={(checked) => setOffer(offer.id,{pubblicata:checked})} />Pubblicata</label><div className="flex gap-2">{offer.url_pubblico ? <Button variant="outline" size="sm" render={<a href={offer.url_pubblico} target="_blank" rel="noreferrer" />}>Apri link</Button> : null}{offer.pdf_path ? <Button variant="outline" size="sm" render={<a href={`/api/public/offerte-periodo/${offer.id}/pdf`} target="_blank" rel="noreferrer" />}>Apri pubblico</Button> : null}{offer.pdf_path ? <Button variant="outline" size="sm" render={<a href={`/api/offerta-commerciale/offerte/${offer.id}/asset`} target="_blank" rel="noreferrer" />}>Apri PDF</Button> : null}{canManage ? <Button size="sm" onClick={() => saveOffer(offer)}>Salva</Button> : null}</div></div></div></article>)}</div>}</TabsContent>
