...
export function OffertaCommercialeClient() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <Tabs defaultValue="calcolo">
      ...
      <TabsContent value="offerte" className="space-y-4 pt-5">{data.offerte.length === 0 ? <Empty>Carica PDF e copertine da Documenti, riquadro “Offerte del periodo”.</Empty> : <div className="grid gap-4 lg:grid-cols-2">{data.offerte.map((offer) => <article key={offer.id} className="overflow-hidden rounded-xl border border-border bg-card">{offer.cover_path ? <img src={`/api/offerta-commerciale/offerte/${offer.id}/asset?kind=cover`} alt="" className="h-52 w-full object-cover" /> : <div className="flex h-36 items-center justify-center bg-muted"><FileText className="size-10 text-muted-foreground" /></div>}<div className="space-y-3 p-4"><div className="grid gap-2 sm:grid-cols-[1fr_150px]"><Input value={offer.titolo} disabled={!canManage} onChange={(e) => setOffer(offer.id,{titolo:e.target.value})} /><select className="h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground" value={offer.tipo ?? "offerta"} disabled={!canManage} onChange={(e) => setOffer(offer.id,{tipo:e.target.value as OffertaPeriodo["tipo"]})}><option value="offerta">Offerta</option><option value="locandina">Locandina</option><option value="brochure">Brochure</option><option value="finanziaria">Finanziaria</option><option value="pagina">Pagina</option></select></div><Textarea value={offer.descrizione ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{descrizione:e.target.value})} placeholder="Descrizione breve per Roberta" /><div className="grid grid-cols-2 gap-2"><Input type="date" value={offer.valido_dal ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{valido_dal:e.target.value || null})} /><Input type="date" value={offer.valido_al ?? ""} disabled={!canManage} onChange={(e) => setOffer(offer.id,{valido_al:e.target.value || null})} /></div><div className="flex items-center justify-between gap-2"><label className="flex items-center gap-2 text-sm"><Switch checked={offer.pubblicata} disabled={!canManage} onCheckedChange={(checked) => setOffer(offer.id,{pubblicata:checked})} />Pubblicata</label><div className="flex gap-2">{!offer.pdf_path && offer.url_pubblico ? <Button variant="outline" size="sm" render={<a href={offer.url_pubblico} target="_blank" rel="noreferrer" />}>Apri link</Button> : null}{offer.pdf_path ? <Button variant="outline" size="sm" render={<a href={`/api/offerta-commerciale/offerte/${offer.id}/asset`} target="_blank" rel="noreferrer" />}>Anteprima PDF</Button> : null}{offer.pdf_path && offer.pubblicata ? <Button variant="outline" size="sm" render={<a href={`/api/public/offerte-periodo/${offer.id}/pdf`} target="_blank" rel="noreferrer" />}>Apri pubblico</Button> : null}{canManage ? <Button size="sm" onClick={() => saveOffer(offer)}>Salva</Button> : null}</div></div></div></article>)}</div>}</TabsContent>
