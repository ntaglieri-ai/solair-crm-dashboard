...
type TagAction =
  | { action: "toggle"; installatoreId: string; tagId: string; enabled: boolean }
  | { action: "create"; names: string[]; color: string }
  | { action: "create_assign"; installatoreId: string; name: string; color: string }
  | { action: "update"; tagId: string; name?: string; color?: string }
  | { action: "delete"; tagId: string }

export async function GET() {
  const guard = await requireApiPage("installatori")
  if (guard.response) return guard.response

  try {
    return NextResponse.json(await loadInstallatoreReferenceData())
  } catch (error) {
...
export async function POST(request: Request) {
  const guard = await requireApiAction("installatori.tags.edit")
  if (guard.response) return guard.response
  ...
  if (body.action === "create") {
    ...
    const rows = names
      ...
      .map((nome) => ({ nome, colore: body.color || "#64748b", modulo: "installatore" }))
  ...
  if (body.action === "toggle") {
    if (!body.installatoreId || !body.tagId) {
      return NextResponse.json({ error: "Installatore o tag non valido" }, { status: 400 })
    }
    const query = supabase
      .from("installatore_tags")
      .delete()
      .eq("installatore_id", body.installatoreId)
      .eq("tag_id", body.tagId)
    const { error } = body.enabled
      ? await supabase
          .from("installatore_tags")
          .upsert(
            { installatore_id: body.installatoreId, tag_id: body.tagId },
            { onConflict: "installatore_id,tag_id" },
          )
      : await query
  ...
  if (body.action === "create_assign") {
    ...
    if (!name || !body.installatoreId) {
      return NextResponse.json({ error: "Nome tag o installatore non valido" }, { status: 400 })
    }
    ...
    if (!tag) {
      const created = await supabase
        .from("tag")
        .insert({ nome: name, colore: body.color || "#64748b", modulo: "installatore" })
        .select("id,nome,colore,created_at")
        .single()
      if (created.error) {
        return NextResponse.json({ error: created.error.message }, { status: 500 })
    ...
    const assigned = await supabase
      .from("installatore_tags")
      .upsert(
        { installatore_id: body.installatoreId, tag_id: tag.id },
        { onConflict: "installatore_id,tag_id" },
      )
    if (assigned.error) {
