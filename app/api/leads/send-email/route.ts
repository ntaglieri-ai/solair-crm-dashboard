...
export async function POST(request: Request) {
  ...
  const leadIds = Array.isArray(payload?.leadIds)
    ? payload.leadIds.filter((id): id is string => typeof id === "string")
    : []
  ...
  const recipients = (leads ?? [])
    .map((lead) => (lead as { email: string | null }).email)
    .filter((email): email is string => Boolean(email && email.includes("@")))

  if (recipients.length === 0) {
    return NextResponse.json(
      { error: "Nessuno dei lead selezionati ha un indirizzo email valido." },
      { status: 400 },
    )
  }
