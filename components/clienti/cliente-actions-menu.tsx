...
type UpdateField = "Stato" | "Sede" | "Tag"

const MOCK_DRAFTS = [
  {
    id: "d1",
    subject: "Aggiornamento pratica incentivi",
    snippet: "Gentile cliente, le confermiamo lo stato della pratica…",
    updated: "Ieri, 17:42",
  },
  {
    id: "d2",
    subject: "Promemoria saldo impianto",
    snippet: "Buongiorno, le ricordiamo la scadenza del saldo…",
...
export function ClienteActionsMenu({
  ...
  selectedRows: ClienteRecord[]
  tags: string[]
  onOpenSettings: (section: ClienteSettingsSectionId) => void
  ...
  const dupGroups = useMemo(() => {
    ...
    for (const c of selectedRows) {
      const keys = [norm(c["E-mail"]), norm(c.Cellulare)].filter(Boolean)
    ...
    const groups: { key: string; rows: ClienteRecord[] }[] = []
    ...
    <>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Invia email ai clienti filtrati</DialogTitle>
            <DialogDescription>
            ...
            <Button
              ...
              onClick={() => {
                toast.success("Email in invio", {
                  description: `Invio a ${filtered.length} clienti avviato.`,
                })
                setDialog("none")
              }}
            >
      ...
      >
        <DialogContent className="flex h-[90vh] max-h-[90vh] w-[96vw] max-w-[96vw] flex-col gap-0 p-0">
          <DialogHeader className="border-b border-border px-4 py-3">
            ...
            <DialogDescription>
              {filtered.length} clienti — visualizzazione foglio a schermo
              intero.
            </DialogDescription>
          </DialogHeader>
            ...
            <table className="w-full border-collapse text-sm">
              <thead className="sticky top-0 z-10 bg-muted text-left">
                <tr className="border-b border-border">
                  {["Nome Clienti", "Stato", "Sede", "Clienti Proprietario", "Installatore", "E-mail"].map(
                    (h) => (
                      <th
                        key={h}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Assegna i {selectedCount} clienti selezionati a un nuovo
              proprietario.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Aggiorna un campo su tutti i {selectedCount} clienti selezionati.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>De-duplica clienti</DialogTitle>
            <DialogDescription>
              {dupGroups.length > 0
                ? "Per ogni gruppo duplicato scegli il record da mantenere; gli altri verranno eliminati."
                : "Nessun duplicato rilevato tra i clienti selezionati."}
            </DialogDescription>
          </DialogHeader>
          {dupGroups.length > 0 ? (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="text-destructive">
              Questa azione eliminerà {selectedCount} clienti. Non può essere
              annullata.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
