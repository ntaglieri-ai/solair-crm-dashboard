...
import {
  ...
  isLightColor,
  type ClienteTag,
} from "@/lib/cliente-tag-store"

/* -------------------------------------------------------------------------- */
/*                          Pill a freccia (chevron)                          */
/* -------------------------------------------------------------------------- */

function ClienteTagPill({ tag }: { tag: ClienteTag }) {
...
function AddTagDialog({
  ...
  const { createTags } = useClienteTags()
  const [value, setValue] = useState("")
  const [color, setColor] = useState<string>(CLIENTE_TAG_PALETTE[13])

  const reset = () => {
    setValue("")
    setColor(CLIENTE_TAG_PALETTE[13])
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-xs text-muted-foreground">
            Tag utilizzati: {tags.length} / {MAX_CLIENTE_TAGS}
          </span>
        </div>
      </div>
...
export function ClienteTagSection() {
  return (
    <ClienteTagProvider>
      <TagManager />
    </ClienteTagProvider>
