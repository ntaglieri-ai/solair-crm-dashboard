...
// Tag delle Scadenze: campo testo libero (scadenze.tag), nessun significato
// predefinito — implementazione autonoma, non condivide storage/infra con
// lib/tag-store.tsx (quello è multi-tag per i Lead; qui è un valore singolo
// per record). Colore derivato via hash deterministico, non persistito.
...
export function scadenzaTagColor(name: string): string {
  let hash = 0
  for (let i = 0; i < name.length; i++) {
    hash = (hash << 5) - hash + name.charCodeAt(i)
    hash |= 0
  }
  return SCADENZA_TAG_PALETTE[Math.abs(hash) % SCADENZA_TAG_PALETTE.length]
}
...
export function ScadenzaTagDot({ color, className }: { color: string; className?: string }) {
  return (
    <span
...
export function ScadenzaTagChip({ tag, className }: { tag: string | null; className?: string }) {
  ...
  return (
    <span
      ...
      style={tagStyle(scadenzaTagColor(tag))}
    >
...
function ScadenzaTagPickerList({
  value,
  suggestions,
  onSelect,
  onClear,
    ...
    <div className="flex w-full flex-col">
      ...
      <div className="flex max-h-56 flex-col gap-0.5 overflow-y-auto py-1.5">
        {filtered.map((tag) => {
          ...
          const color = scadenzaTagColor(tag)
          return (
            <button
              key={tag}
              type="button"
      ...
      {canCreate ? (
        <div className="flex items-center gap-2 border-t border-border px-1 pt-2">
          ...
          >
            ...
            <ScadenzaTagDot color={scadenzaTagColor(query.trim())} className="ml-auto" />
          </button>
        </div>
      ) : null}
...
/** Campo tag per form (crea/modifica scadenza): bottone + popover di selezione. */
export function ScadenzaTagField({
  value,
  onChange,
  suggestions,
}: {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        render={
          ...
          >
            {value ? <ScadenzaTagChip tag={value} /> : <span className="text-muted-foreground">Nessun tag</span>}
            <ChevronDown size={16} className="shrink-0 text-muted-foreground" />
          </button>
        }
      />
      <PopoverContent align="start" className="w-full min-w-64 gap-0 p-2">
        <ScadenzaTagPickerList
          value={value || null}
 ...
 */
export function ScadenzaTagMenuDialog({
  value,
  suggestions,
  onSelect,
  open,
