...
Il logout CRM cancella la sessione CRM e l'eventuale handoff pendente; non invia
piu' una fetch cross-origin a `/logout` (richiede un token CSRF Nextcloud e puo'
arrivare in ritardo dopo un nuovo login). Il cambio identita' Nextcloud avviene
alla successiva apertura dal CRM. Per chiudere subito anche una scheda Nextcloud
gia' aperta, usare il suo logout. Sono preservate la pagina `/nextcloud/login`
e la root limitata alle condivisioni esplicite per gli agenti.
