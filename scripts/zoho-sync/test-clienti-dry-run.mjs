...
const defaultCsv = "/Users/imacnando/Downloads/Clienti_2026_08_15.csv"
...
function compileZohoSyncLib() {
  const outDir = path.join(repoRoot, ".next/cache", `zoho-sync-clienti-${process.pid}-${Date.now()}`)
  const args = [
    "exec",
    "tsc",
    "--target",
...
function printSummary(result, logToDatabase) {
  ...
  console.log(JSON.stringify({
    ...
    unresolvedInstallatoreIds: stats.unresolvedInstallatoreIds ?? [],
...
const { runClientiDryRun } = require(runnerPath)
const supabase = createClient(supabaseUrl, serviceRoleKey, {
  auth: { persistSession: false, autoRefreshToken: false },
})

const result = await runClientiDryRun({ csvPath, supabase, logToDatabase })
