...
const defaultCsv = "/Users/imacnando/Downloads/Compiti_2026_08_15.csv"
...
function compileZohoSyncLib() {
  const outDir = path.join(repoRoot, ".next/cache", `zoho-sync-compiti-${process.pid}-${Date.now()}`)
  const args = [
    "exec",
    "tsc",
    "--target",
...
const { runCompitiDryRun } = require(runnerPath)
const supabase = createClient(supabaseUrl, serviceRoleKey, {
  auth: { persistSession: false, autoRefreshToken: false },
})

const result = await runCompitiDryRun({ csvPath, supabase, logToDatabase })
