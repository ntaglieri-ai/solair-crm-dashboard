...
const defaultCsv = "/Users/imacnando/Downloads/Installatori_2026_08_15.csv"
...
function compileZohoSyncLib() {
  const outDir = path.join(repoRoot, ".next/cache", `zoho-sync-installatori-${process.pid}-${Date.now()}`)
  const args = [
    "exec",
    "tsc",
    "--target",
...
const { runInstallatoriDryRun } = require(runnerPath)
const supabase = createClient(supabaseUrl, serviceRoleKey, {
  auth: { persistSession: false, autoRefreshToken: false },
})

const result = await runInstallatoriDryRun({ csvPath, supabase, logToDatabase })
