...
const defaultCsv = "/Users/imacnando/Downloads/Scadenze_2026_08_15.csv"
...
function compileZohoSyncLib() {
  const outDir = path.join(repoRoot, ".next/cache", `zoho-sync-scadenze-${process.pid}-${Date.now()}`)
  const args = [
    "exec",
    "tsc",
    "--target",
...
const { runScadenzeDryRun } = require(runnerPath)
const supabase = createClient(supabaseUrl, serviceRoleKey, {
  auth: { persistSession: false, autoRefreshToken: false },
})

const result = await runScadenzeDryRun({ csvPath, supabase, logToDatabase })
